...
def cogvlm_caption_image(image, prompt="Please describe this video in detail."):
    import rp.web_evaluator as wev

    client = wev.Client("100.113.27.133")
    result = client.evaluate("if not 'ffmpeg' in get_system_commands():os.system('sudo apt install ffmpeg --yes')", x=image, prompt=prompt)
    result = client.evaluate("run_captioner(save_video_mp4([x]*3),prompt=prompt)", x=image, prompt=prompt)
    if result.errored:
        raise result.error
    return result.value
...
if __name__ == "__main__":
    ...
    fansi_print("Running CogVLM Captioner...","orange","bold",new_line=False)
    caption = cogvlm_caption_image(image)
    fansi_print("Default Caption: "+repr(caption),"orange","bold")
    prompt=input(fansi('Input your own caption, or leave blank to use default >>> ','blue cyan','bold italic underlined')) or caption
    fansi_print("Using Caption: "+repr(prompt),"blue cyan","italic")
    ...
    should_warp_noise=input_yes_no(fansi('Warp noise locally on this machine?','blue','bold underlined'))
    ...
    output_video_file=save_video_mp4(output_frames, output_folder+'/'+title + ".mp4", video_bitrate="max",framerate=12)

    mask_video = as_numpy_array(
        [
    ...
    np.save(output_folder + "/" + title + "_mask.npy", mask_video.astype(bool))
    ...
    if should_warp_noise:
        ...
        def compress_cartridge(cartridge):
            video=cartridge['instance_video']/2+.5
            video=encode_images_to_bytes(as_numpy_images(video))
            cartridge['instance_video']=video
        compress_cartridge(cartridge) #Make the video smaller, from 100MB or more to just 20MB

        output_cartridge_file=object_to_file(cartridge, output_folder + "/" + title + "__compressed_cartridge.pkl")
    ...
    polygons=list(map(as_numpy_array,layer_polygons))
    ...
    if should_warp_noise:print(fansi('    - Saved cartridge: ','green','bold'),fansi_highlight_path(output_cartridge_file))
